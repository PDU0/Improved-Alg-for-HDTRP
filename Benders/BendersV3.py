...
def BendersV3(test, name, time_limit, CWD, param='new'):
    # Benders approch
    ...
    logName = CWD + r'\Benders\BendersTestLog\BendersV3-%s.txt' %(name)
    ...
    plotPath = CWD + r'\Benders\BendersTestLog\BendersV3-{}.png'.format(name)
...
def main(): # If you want batch testing, go to test.py
    ...
    name = 'A-n32-k5'
    test = load_test('A', TestDir)[name]
    ##########################################################################
    t0 = time.time()
    t,_,_,_,_ = BendersV3(test, name, timeLimit, CWD)
    t1 = time.time()
